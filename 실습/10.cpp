...
void TimerFunction(int value) {
    ...
    if (four) {
        for (int i = 0; i < total; i++) {
            ...
            // 상태 전환 조건
            if (disnt <= 0.01f) shapes[i].state = 1, disnt = 0.02f, shapes[i].alpha = 0.0f; // 바깥으로
            if (disnt >= 1.1f) shapes[i].state = 0, disnt = 1.09f, shapes[i].alpha = 0.0f; // 안으로

            // radian를 각도로, alpha를 반지름 조정으로 사용
            // 새로운 중심점 계산 (나선형)
            ...
            // 모든 정점을 동일하게 이동 (삼각형 형태 유지)
            for (int j = 0; j < shapes[i].vertices.size(); j += 3) {
                shapes[i].vertices[j] += deltaX;     // X 좌표
                shapes[i].vertices[j + 1] += deltaY; // Y 좌표
            }

            // 상태에 따른 값 업데이트
            if (shapes[i].state == 0) { // 안으로 나선
                shapes[i].radian += 0.1;    // 각도 증가 (빠른 회전)
                shapes[i].alpha -= 0.0001f; // 반지름 감소
            }
            if (shapes[i].state == 1) { // 바깥으로 나선
                shapes[i].radian -= 0.1;    // 각도 감소 (반대 회전)
                shapes[i].alpha += 0.0001f; // 반지름 증가
            }
