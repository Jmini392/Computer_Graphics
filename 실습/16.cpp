...
glm::mat4 corMatrix(1.0f);
glm::vec3 axis(0.0f);
bool showcube = true, showcorn = false, wire = false;
bool rotate = false, depth = false, curling = false;
float angle = 0.0f;
...
void CreateMatrix();
void RotateMatrix(float angle, glm::vec3 axis);
...
// 회전 행렬 갱신 (자전)
void RotateMatrix(float angle, glm::vec3 axis) {
    // 현재 객체의 위치를 추출 (변환 행렬의 4번째 열)
    glm::vec3 currentPos = glm::vec3(Matrix[3][0], Matrix[3][1], Matrix[3][2]);

    // 1. 객체를 원점으로 이동
    glm::mat4 translateToOrigin = glm::translate(glm::mat4(1.0f), -currentPos);

    // 2. 원점에서 회전
    glm::mat4 rotMat = glm::rotate(glm::mat4(1.0f), glm::radians(angle), axis);

    // 3. 다시 원래 위치로 이동
    glm::mat4 translateBack = glm::translate(glm::mat4(1.0f), currentPos);

    // 변환 순서: 원래위치로이동 * 회전 * 원점으로이동 * 기존행렬
    Matrix = translateBack * rotMat * translateToOrigin * Matrix;
}
...
GLvoid SpecialKeyboard(int key, int x, int y) {
    switch (key) {
    case GLUT_KEY_UP: // 위쪽 이동
        TranslateMatrix(0.0f, 0.1f, 0.0f);
		break;
    case GLUT_KEY_LEFT: // 왼쪽 이동
        TranslateMatrix(-0.1f, 0.0f, 0.0f);
		break;
    case GLUT_KEY_RIGHT: // 오른쪽 이동
        TranslateMatrix(0.1f, 0.0f, 0.0f);
		break;
    case GLUT_KEY_DOWN: // 아래쪽 이동
        TranslateMatrix(0.0f, -0.1f, 0.0f);
		break;
    }
    glutPostRedisplay();
...
// 타이머 콜백 함수
void TimerFunction(int value) {
    if (rotate) RotateMatrix(angle, axis);
	glutPostRedisplay();
